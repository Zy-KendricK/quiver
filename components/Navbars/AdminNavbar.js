...
export default function Navbar() {
  return (
    <>
      {/* Navbar */}
      <nav className="fixed top-0 left-64 w-11/12 z-10 bg-transparent md:flex-row md:flex-nowrap bg-white md:justify-start flex items-center md:border-b md:flex hidden">
        <div className="grid grid-cols-4 divide-x w-full">
          <div className="w-full p-3.5  mx-auto col-span-3 items-center flex justify-between md:flex-nowrap flex-wrap md:px-10 px-4  border-r">
            {/* Brand */}
            {/* Search Form */}
            <form className="md:flex hidden w-full flex-row flex-wrap items-center mr-3">
              <div className="relative flex w-full flex-wrap items-stretch">
                ...
                <input
                  ...
                  className="border-0 px-3 py-4 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-sm outline-none focus:outline-none focus:ring focus:ring-green-600 w-full pl-10"
                />
              </div>
            </form>
            {/* Notification */}
            <div className="relative flex w-full flex-wrap items-stretch md:justify-end">
              <div className="z-10 h-full leading-snug font-normal absolute text-center text-blueGray-300 absolute bg-transparent rounded text-base items-center justify-center w-8 pl-3 -mt-2">
          ...
          <div>
            {/* User */}
            <ul className="flex-col p-3.5 md:flex-row list-none items-center hidden md:flex ml-6">
              <UserDropdown />
              <ul className="px-3">
